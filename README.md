...
├── triage_grader_v2.py      # Reward computation logic (penalties + partial credit)
...
| Secondary intent bonus | **+0.10** | Hard tasks only — catching secondary category (+0.05) and route (+0.05) |
| Secondary intent ignored | **-0.05** | Penalty per field when ground truth exists but agent returns None |
| Secondary intent wrong | **-0.02** | Lighter penalty per field when agent guesses incorrectly |

**Total reward range: `0.0 – 1.0`** (clamped)
...
```json
{
  ...
  "done": false,
  "info": {
    "grading": { "priority": 0.35, "category": 0.35, "route": 0.30, "secondary": 0.0 },
